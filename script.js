...
function ReadAllTask() {
    ...
    for(var i = 0; i < mytodo_list.length; i++) {
        ...
        data += "<td><button class='btn btn-primary' onclick='UpdateTask(" + i + ")'>Actualizar</button></td>";
        data += "<td><button class='btn btn-danger' onclick='DeleteTask(" + i + ")'>Borrar</button></td>";
        data += "</tr>";
    }
    document.getElementById("counter").innerHTML = mytodo_list.length + " Tareas";
